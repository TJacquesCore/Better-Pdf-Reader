    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->sidebarFrame->setStyleSheet("background-color: transparent;");
    ui->mainFrame->setStyleSheet("background-color: transparent;");

    // Donne un fond sombre semi-transparent (le Mica adore ça)
    // Valeur typique pour un look Fluent : gris foncé ~20-30% opacité
    setStyleSheet("QMainWindow { background: rgba(32, 32, 32, 180); }");
    ...
    //connect pour jump
    connect(ui->actual_page, &QLineEdit::editingFinished, this , &MainWindow::jumpToPage);
...
{
    int nextpage=ui->actual_page->text().toInt();
    if(nextpage > 0 && nextpage <= document->pageCount())
        view->pageNavigator()->jump(nextpage - 1, QPointF(), 0);
...
void MainWindow::updateLabel()
{
    ...
    ui->actual_page->setText(QString("%1").arg(current));
    ui->pageInfoLabel->setText(QString("Page %1").arg(total));
}
